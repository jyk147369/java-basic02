...
public class JavaTalkServerThread extends Thread{
    ...
    // 스레드 생성자 만들기
    public JavaTalkServerThread(Socket socket, String id) {
    ...
    public void run() {
        ...
        try {
            ...
            while(true) {
                ...
                // 클라이언트 소켓 리스트만큼 반복
                for (int i = 0; i < JavaTalkServerMain.clientIdSocketMap.size(); i++) {
                    Socket client = JavaTalkServerMain.clientIdSocketMap.get(i);
                    // 클라이언트소켓에게 보낼거임
