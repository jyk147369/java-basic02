...
public class JavaTalkServerMain {
    ...
    static List<Socket> clientSocketList = new ArrayList<Socket>();
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while(true){
                ...
                clientSocketList.add(cs);                             // 소켓리스트에 클라이언트소켓을 추가

                System.out.println(cs + "클라이언트와 연결되었습니다.");
                System.out.println();

                // 클라이언트와 연결되면 클라이언트의 채팅을 순차적으로 여러번 받는 스레드를 실행시켜준다.
                Thread javaTalkServerThread = new JavaTalkServerThread(cs);   // JavaTalkServerThread 클래스를 호출해서 javaTalkServerThread 생성자의 매개변수에 클라이언트소켓을 저장하여 객체를 생성
